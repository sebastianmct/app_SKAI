...
class ExternalProductRepositoryTest : StringSpec({
    ...
    "Cuando obtengo productos externos, debe convertirlos al modelo interno" {
        runTest {
            ...
            coEvery { externalApiService.getAllProducts() } returns Response.success(externalProducts)
            ...
            result.first().id shouldBe "external_1"
            result.first().name shouldBe "Fjallraven Backpack"
            result.first().category shouldBe "Ropa"
            result.first().price shouldBe 109950.0
        }
    }
